...
const ContentFactory: React.FC<ContentFactoryProps> = ({ activeClient }) => {
  ...
  const handleGenerateText = async () => {
    ...
    const data = await generateSocialContent(topic, activeClient.voice, platforms);
    setTextResults(data);
      ...
      <div className="flex border-b border-slate-200 space-x-8">
        {[
            ...
            <button
                ...
                className={`flex items-center space-x-2 pb-4 border-b-2 font-medium transition-all ${
                    activeTab === tab.id 
